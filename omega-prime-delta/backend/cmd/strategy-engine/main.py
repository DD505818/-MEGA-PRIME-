...
agents = [Agent001(), Agent002(), Agent003(), Agent004(), Agent005(), Agent006(),
          Agent007(), Agent008(), Agent009(), Agent010(), Agent011(), Agent012(),
          Agent013(), Agent014(), Agent015()]


async def process():
    for msg in consumer:
        tick = json.loads(msg.value)
        context = {'bars': [], 'equity': 10000}  # would fetch from portfolio
        for agent in agents:
