...
func main() {
	...
	for {
		...
		default:
			...
			if !idempotency.TryLock(order.ID) {
				log.Printf("Duplicate order rejected: %s", order.ID)
			...
			if err := executedWriter.WriteMessages(ctx, kafka.Message{Value: fillBytes}); err != nil {
				...
				log.Printf("Order executed: %s -> fill %s", order.ID, fill.FillID)
			}
		}
	}
}
...
func rejectOrder(writer *kafka.Writer, order models.Order, reason string) {
	...
	rejection := models.OrderRejection{OrderID: orderID, Reason: reason, Timestamp: time.Now().UnixMilli()}
